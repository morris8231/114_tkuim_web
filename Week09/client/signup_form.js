...
form.addEventListener('submit', async (event) => {
  ...
  if (!(okName && okEmail && okPhone && okPwd && okConfirm && okTos)) {
    const firstInvalid = form.querySelector(':invalid');
    if (firstInvalid) firstInvalid.focus();
    return;
  }

  // 組出要送給後端的資料（欄位名稱可依後端調整）
  const payload = {
  ...
  try {
    ...
    showPageAlert('success', data.message || '註冊成功！資料已送出。');
    ...
    console.error(error);
    showPageAlert('danger', '無法連線到伺服器，請確認伺服器是否有啟動。');
  } finally {
